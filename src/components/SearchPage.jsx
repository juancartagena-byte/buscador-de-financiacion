...
export default function SearchPage({ openPanel }) {
  ...
  const resultFunds = results
    ? (results.funds || [])
        .map((r) => ({ result: r, fund: funds.find((f) => f.id === r.id) }))
        .filter((x) => x.fund)
    : [];
    ...
    >
      ...
      >
        {searchState === "home" && (
          <div className="hero">
            <h1>
              Encuentra la <em>fuente ideal</em> para tu necesidad
            </h1>
            <p>
              Describe tu situación en lenguaje natural y te mostraremos las 8
              fuentes de financiación más compatibles con tu solicitud.
            </p>
          </div>
        )}

        <div className="search-card">
          <label className="search-label" htmlFor="queryInput">
            Describe tu necesidad de financiación
          </label>
          <textarea
            id="queryInput"
            ref={textareaRef}
          ...
          <div className="search-card-row">
            <div className="filter-budget-wrap">
              <span className={`prefix${rawBudget ? " active" : ""}`}>
                COP $
              </span>
              <input
                type="text"
      ...
      {searchState === "loading" && (
        <div className="loading-section">
          ...
          <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 8 }}>
            Comparando con 32 fuentes de financiación
          </p>
        </div>
      )}
          ...
          {budgetDisplay && (
            <div className="filter-pills">
              <span className="filter-pill">
                💰 COP ${budgetDisplay.cop} {budgetDisplay.usd}
              </span>
            </div>
          )}
          <div className="results-count">
            {resultFunds.length} fondos recomendados
          </div>

...
export default function Dashboard({ openPanel }) {
    ...
    function handleClick(e) {
      ...
      for (const slice of pieSlicesRef.current) {
        ...
        if (angle <= slice.endAngle) {
          setSelectedType((prev) => prev === slice.label ? null : slice.label)
          return
        }
      }
  ...
  function drawBarChart(canvas, data) {
    ...
    const counts = BAR_LABELS.map((_, i) => data.filter((f) => f.p[i] === 1).length)
  ...
  function drawPieChart(canvas, activeLabel) {
    ...
    const data = PIE_DATA
      .map((d) => ({ ...d, count: funds.filter((f) => f.t === d.label).length }))
      .filter((d) => d.count > 0)
    ...
    data.forEach((d) => {
      ...
      ctx.beginPath()
      ctx.moveTo(cx + ox + inner * Math.cos(angle), cy + oy + inner * Math.sin(angle))
      ...
      pieSlicesRef.current.push({
        ...
        cx, cy, r: radius + EXPLODE + 4, inner: inner - 4,
      })
    ...
    data.forEach((d) => {
      ...
      ctx.fillStyle     = '#1a1a1a'
      ctx.font          = isActive ? "bold 13px 'Source Sans 3',sans-serif" : "13px 'Source Sans 3',sans-serif"
      ctx.textAlign     = 'left'
      ctx.textBaseline  = 'middle'
    ...
    <div style={{ paddingTop: 64, minHeight: '100vh' }}>
      <div className="dash-container">
        <div className="dash-header">
          <h1>Dashboard de <em>Fuentes</em></h1>
          <p>Resumen analítico de las {total} fuentes de financiación para gestión del riesgo y cambio climático disponibles en el sistema.</p>
        </div>

        {/* Summary cards */}
        <div className="dash-cards">
          {[
            { num: total,              lbl: 'Total de fuentes',  color: 'var(--green)', w: 100 },
            { num: nacCount,           lbl: 'Nacionales',        color: '#1e6b3a', w: (nacCount / total) * 100 },
            { num: terCount,           lbl: 'Territoriales',     color: '#8b5e1a', w: (terCount / total) * 100 },
            { num: intCount,           lbl: 'Internacionales',   color: '#4a2d82', w: (intCount / total) * 100 },
            { num: prvCount + otrCount, lbl: 'Privado / Otro',   color: '#1a5fa0', w: ((prvCount + otrCount) / total) * 100 },
          ].map((s) => (
            <div className="dash-stat" key={s.lbl}>
              <div className="num" style={{ color: s.color }}>{s.num}</div>
              <div className="lbl">{s.lbl}</div>
              <div className="bar" style={{ background: s.color, width: s.w + '%' }} />
            </div>
          ))}
        </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
              ...
              {selectedType && (
                <span style={{
                  ...
                  background: typeBg(selectedType), color: typeColor(selectedType),
        ...
        <div className="dash-table-card">
          ...
          <div className="dash-table-wrap">
            <table className="dash-table">
              <thead>
                <tr>
                  <th>ID</th><th>Nombre</th><th>Tipo</th><th>Monto</th><th>Vigencia</th>
                </tr>
              </thead>
              <tbody>
                {filteredFunds.map((f) => (
                  <tr key={f.id} style={{ cursor: 'pointer' }} onClick={() => openPanel(f.id)}>
                    <td style={{ fontWeight: 700, color: 'var(--text-light)' }}>{f.id}</td>
                    <td>
                      <strong>{f.s}</strong><br />
                      <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{f.n}</span>
                    </td>
                    <td>
                      <span className="tipo-badge" style={{ background: typeBg(f.t), color: typeColor(f.t) }}>
                        {f.t}
                      </span>
                    </td>
